...
import GA (Entity(..), GAConfig(..), evolve, evolveVerbose)
...
-- Hier staat nu "IO" ipv Identity
 
  -- generate a random entity, i.e. a random integer value 
  genRandom _ seed = return $ (fst $ random $ mkStdGen seed) `mod` 10000
  -- mkStdGen(x) voor elke x, geeft dit een functie (settheoric), waarbij de output een random nummer is.
  ...
  --isPerfect(_,s) = s == 0
  -- deze toevoegen als je wil dat die stopt als er een perfect resultaat is
...
main = do
        let cfg = GAConfig 
            ...
            g = mkStdGen 1 -- random generator
            -- de head van (randoms g) is het argument 'seed' in genRandom.
            -- randoms g, is een lijst van random getallen, gecreerd in package GA,
            -- door de standaard functie 'randoms' uit System.Random, die type heeft:
            -- geef me een randomgenerator en ik spuug een lijst random getallen uit.
            
        es <- evolveVerbose g cfg () ()
        -- Hier nu <- ipv =; en uitgelijnd naar links
        let e = snd $ head es :: Int
