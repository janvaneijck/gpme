...
performCrossover crsspara n seed pool es = do 
    let g = mkStdGen seed
        (selSeeds,seeds) = takeAndDrop (2*2*n) $ randoms g
        -- de `eerste' 2 hierboven is om er mee om te kunnen gaan dat elk stel maar
        -- één kind kan krijgen; de `tweede' 2, is een safety mechanism, omdat je 
        -- het zo zou kunnen maken dat sommige stellen geen kinderen kunnen krijgen.
        -- Het moet zo zijn dat minstens 50% vd stellen voortplant
        (crossSeeds,_) = takeAndDrop (2*n) seeds
        tuples = currify $ map (tournamentSelection es) selSeeds
        -- tournementSelection geeft een willkeurig (gewogen) organisme
        -- "map (tournamentSelection es) selSeeds" geeft een lijst van organismen, let
        -- op hier komen organismen dubbel in voor.
        -- curify maakt stellen van de geselecteerde ognaismen.        
        
    resEntities <- zipWithM ($) 
                     (map (uncurry . (crossover pool crsspara)) crossSeeds) 
                     tuples
        -- dit is een lijst met Maybe's van de resulting entities
...
initGA g cfg pool = do
    ...
    let (seed:rs) = randoms g :: [Int]
        -- de g geef je op in je eigen programma. Daarmee kies je een randomgenerator.
        -- dan kan je met randoms g een lijst random getallen genereren.
        -- het handige hieraan is, dat deze lijst altijd hetzelfde is, voor dezelfde g.
    ...
    let -- number of entities generated by crossover/mutation
        ...
        -- seeds is nu een lijst van het aantal generaties aan random getallen.
        
        -- seeds per generation
        genSeeds = zip [0..] seeds
        -- elke generatie heeft nu geassocieerd met zich een random getal.
